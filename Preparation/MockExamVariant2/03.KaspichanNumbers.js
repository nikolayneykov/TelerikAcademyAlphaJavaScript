var bigInt = (function () {
  ...
  function multiplyKaratsuba (x, y) {
    ...
    var abcd = multiplyKaratsuba(addAny(a, b), addAny(c, d))
    var product = addAny(
    ...
    for (shift = a_l - b_l; shift >= 0; shift--) {
      ...
      if (remainder[shift + b_l] !== divisorMostSignificantDigit) {
        quotientDigit = Math.floor(
          (remainder[shift + b_l] * base +
            remainder[shift + b_l - 1]) /
  ...
  BigInteger.prototype.shiftRight = function (v) {
    ...
    while (n >= powers2Length) {
      if (
        result.isZero() ||
        (result.isNegative() && result.isUnit())
      ) {
        return result
      }
...
for (let i = 97; i <= 122 && digits.length < 256; i++) {
  for (let j = 65; j <= 90 && digits.length < 256; j++) {
...
let result = []
let num = bigInt(gets())
let divisor = bigInt(256)

do {
  result.unshift(digits[num.mod(divisor)])
